...
contract SupplyChain is Ownable, FarmerRole, DistributorRole, RetailerRole, ConsumerRole {
  ...
  modifier onlyOwner() {
    require(msg.sender == owner, "Has to be the owner of the contract.");
    _;
  }

  // Define a modifer that verifies the Caller
  modifier verifyCaller (address _address) {
    require(msg.sender == _address, "The caller has to be a member of the contract.");
    _;
  }

  // Define a modifier that checks if the paid amount is sufficient to cover the price
  modifier paidEnough(uint _price) {
    require(msg.value >= _price, "The value has to be more or equal to the price.");
    _;
  }
  ...
  modifier harvested(uint _upc) {
    require(_hasState(_upc,State.Harvested), "The item has to be harvested.");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Processed
  modifier processed(uint _upc) {
    require(_hasState(_upc,State.Processed), "The item has to be processed.");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Packed
  modifier packed(uint _upc) {
    require(_hasState(_upc,State.Packed), "The item has to be packed.");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is ForSale
  modifier forSale(uint _upc) {
    require(_hasState(_upc,State.ForSale), "The item has to be forSale.");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Sold
  modifier sold(uint _upc) {
    require(_hasState(_upc,State.Sold), "The item has to be sold.");
    _;
  }
  // Define a modifier that checks if an item.state of a upc is Shipped
  modifier shipped(uint _upc) {
    require(_hasState(_upc,State.Shipped), "The item has to be shipped.");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Received
  modifier received(uint _upc) {
    require(_hasState(_upc, State.Received), "The item has to be received.");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Purchased
  modifier purchased(uint _upc) {
    require(_hasState(_upc, State.Purchased), "The item has to be purchased.");
    _;
  }

  // Checks if the item has an especific state.
  function _hasState(uint _upc, State _state) private view returns (bool){
  ...
  // Call modifier to check if upc has passed previous supply chain stage
  // Call modifier to verify caller of this function
  function shipItem(uint _upc) public sold(_upc) verifyCaller(items[_upc].originFarmerID) onlyFarmer
  {
    // Update the appropriate fields
